...
dat_graphs <- dat_incl |> 
  mutate(
    any_figure_contains = case_when(
     spbg_present == "Yes" & contains_adj_spbg == "No" ~ "Grotta bar (Unadjusted)", 
     contains_adj_spbg == "Yes" ~ "Grotta bar (Adjusted)", 
...
filter(neuropathology != "") |> 
  ...
  mutate(
    any_figure_contains = recode(
      ...
      "Unadjusted SPBG" = "Grotta bar (Unadjusted)", 
      "Adjusted SPBG" = "Grotta bar (Adjusted)"
    ),
    any_figure_contains = fct_relevel(any_figure_contains,
                                      "No figures",
                                      "Non-Grotta bar figures only", 
                                      "Grotta bar (Unadjusted)",
                                      "Grotta bar (Adjusted)"), 
...
dat_barplot_stroke <- dat_barplot_stroke |> 
  mutate(
...
barplot_stroke1 <- ggplot(dat_graphs, 
  ...
  scale_fill_manual(values = c("#AE181F",
                               "#CB9698",
                               "#001F83",
                               "#505F81"),
                    limits = c("Grotta bar (Adjusted)", 
                               "Grotta bar (Unadjusted)", 
                               "Non-Grotta bar figures only",
                               "No figures")) +
  ...
  annotate("text", x = 2.35, y = 80, label = "Studies in the sample with a stroke population", size = 4, fontface = "bold") +
  annotate("text", x = 1.35, y = 88, label = "Studies in the sample with a non-stroke population", size = 4, fontface = "bold") +
  scale_y_continuous(limits = c(0, 250))

barplot_stroke1


barplot_stroke2 <- ggplot(dat_barplot_stroke, 
                         aes(x= neuropathology, 
                             fill = any_figure_contains)) + 
  geom_bar(width = 0.5, 
           color = "white", 
